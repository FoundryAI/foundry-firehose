...
class FirehoseServiceFactory {
     ...
     * @property {string} [kinesisConfig.userName] - only necessary if type is 'kinesis'
    ...
    static create(config, logger, basePath) {
        ...
        return new FirehoseService(firehoseClient, logger);
    }
...
class Internals {
    ...
    static createKinesisFirehoseClient(config) {
        const awsFirehose = new AWS.Firehose({
